...
function ChangeBadge({ change, label }: { change: StatChange | undefined; label: string }) {
...
export default function StatCards({ stats }: { stats: DashboardStats }) {
  ...
  const t = dict.dashboardHome.statCards
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <div className="glass-card rounded-xl p-5 border border-gray-800 hover:border-neon-cyan/20 transition-all duration-300 hover:shadow-card-hover">
        <div className="flex items-start justify-between mb-4">
          <p className="text-sm font-medium text-gray-400">{t.totalInvestment}</p>
        ...
        <ChangeBadge change={stats.spendChange} label={t.vsPreviousPeriod} />
      </div>

      {/* ROAS */}
      <div className="glass-card rounded-xl p-5 border border-gray-800 hover:border-neon-cyan/20 transition-all duration-300 hover:shadow-card-hover">
        <div className="flex items-start justify-between mb-4">
          <p className="text-sm font-medium text-gray-400">{t.avgRoas}</p>
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-neon-cyan/10 text-neon-cyan flex-shrink-0">
            <BarChart2 className="w-4 h-4" />
          </div>
        </div>
        <p className="text-2xl font-bold text-white mb-1.5">{fmt(stats.avgRoas, 'roas')}</p>
        <ChangeBadge change={stats.roasChange} label={t.vsPreviousPeriod} />
      </div>

      {/* Conversões */}
      <div className="glass-card rounded-xl p-5 border border-gray-800 hover:border-neon-cyan/20 transition-all duration-300 hover:shadow-card-hover">
        <div className="flex items-start justify-between mb-4">
          <p className="text-sm font-medium text-gray-400">{t.conversions}</p>
          <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-neon-cyan/10 text-neon-cyan flex-shrink-0">
            <ShoppingCart className="w-4 h-4" />
          </div>
        </div>
        <p className="text-2xl font-bold text-white mb-1.5">{fmt(stats.totalConversions, 'number')}</p>
        <ChangeBadge change={stats.conversionsChange} label={t.vsPreviousPeriod} />
      </div>
      ...
      <div className="glass-card rounded-xl p-5 border border-orange-500/30 bg-gradient-to-br from-orange-950/30 to-red-950/20 transition-all duration-300 hover:shadow-card-hover">
        <div className="flex items-start justify-between mb-4">
          <p className="text-sm font-medium text-orange-400">{t.lostRevenue}</p>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-gray-500">{t.pausedCampaigns}</span>
        </div>
      </div>
    </div>
  )
