...
function getScoreLabel(score: number, labels: Dictionary['audit']['funnelHealthReport']['scoreLabels']): string {
...
function getBottleneckBadge(bottleneck: FunnelHealthData['bottleneck'], labels: Dictionary['audit']['funnelHealthReport']['bottleneckLabels']) {
  const map = {
    AD: { label: labels.ad, color: 'bg-purple-500/20 text-purple-400 border-purple-500/30' },
    PRICE: { label: labels.price, color: 'bg-orange-500/20 text-orange-400 border-orange-500/30' },
    SITE: { label: labels.site, color: 'bg-red-500/20 text-red-400 border-red-500/30' },
    MIXED: { label: labels.mixed, color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' },
    HEALTHY: { label: labels.healthy, color: 'bg-green-500/20 text-green-400 border-green-500/30' },
  }
...
function ScoreRing({
  ...
  scoreLabels: Dictionary['audit']['funnelHealthReport']['scoreLabels']
}) {
  const radius = 28
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getScoreColor(score)} bg-current/10`}
        ...
        {getScoreLabel(score, scoreLabels)}
      </span>
    </motion.div>
  )
}
...
export default function FunnelHealthReport({
  ...
  const badge = getBottleneckBadge(data.bottleneck, dict.audit.funnelHealthReport.bottleneckLabels)

  const roasOk = data.roas >= data.targetRoas
  const roasPct = ((data.roas / data.targetRoas - 1) * 100).toFixed(0)
  const roasLabel = roasOk
    ? dict.audit.funnelHealthReport.aboveTarget(roasPct)
    : dict.audit.funnelHealthReport.belowTarget(roasPct)
    ...
    <div className="glass-card rounded-xl border border-gray-800 p-5">
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <h3 className="text-base font-semibold text-white flex items-center gap-2">
            ...
            {dict.audit.funnelHealthReport.title}
          </h3>
      ...
      <div className="mb-5 p-3 rounded-lg bg-white/3 border border-gray-800">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs text-gray-400">{dict.audit.funnelHealthReport.currentRoas}</span>
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-xs text-gray-500">{dict.audit.funnelHealthReport.target} {data.targetRoas}x</span>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <ScoreRing
          score={data.adScore}
          label={dict.audit.funnelHealthReport.ad}
          icon={Megaphone}
          isBottleneck={data.bottleneck === 'AD' || data.bottleneck === 'MIXED'}
          scoreLabels={dict.audit.funnelHealthReport.scoreLabels}
        />
        <ScoreRing
          score={data.priceScore}
          label={dict.audit.funnelHealthReport.price}
          icon={Tag}
        ...
        <ScoreRing
          ...
          label={dict.audit.funnelHealthReport.site}
          icon={Globe}
      ...
      {data.rootCauseInsight && (
        <div className="mb-4 p-3 rounded-lg bg-neon-cyan/5 border border-neon-cyan/20">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-neon-cyan mb-1">{dict.audit.funnelHealthReport.aiRootCause}</p>
      ...
      {data.details.length > 0 && (
        <div>
          ...
          >
            <span>{dict.audit.funnelHealthReport.toggleDetails(showDetails, data.details.length)}</span>
          </button>
