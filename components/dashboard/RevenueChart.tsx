...
import { SkeletonChartCard } from './SkeletonCards';
...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  const roas =
    investment && revenue && investment.value > 0
  ...
  return (
    <div className="bg-dark-card border border-gray-700 rounded-lg px-3 py-2.5 text-xs shadow-xl">
      ...
      {revenue && (
        <p style={{ color: '#00D4FF' }} className="font-semibold mb-0.5">
          Retorno: {formatCurrency(revenue.value)}
        </p>
      )}
      {investment && (
        <p style={{ color: '#8B5CF6' }} className="font-medium mb-0.5">
          Invest.: {formatCurrency(investment.value)}
        </p>
      )}
      <p className="text-gray-400 mt-1">
        ROAS:{' '}
        <span className="text-white font-bold">{roas}x</span>
      </p>
    </div>
  );
}
...
export default function RevenueChart() {
  ...
  const avgRoas = totalRevenue / totalInvestment;
    ...
    <div className="glass-card rounded-xl p-5 border border-gray-800">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Ultimos 30 dias</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <div className="px-3 py-1.5 rounded-lg bg-white/5 border border-gray-700">
          <span className="text-xs text-gray-400">ROAS Medio: </span>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          ...
          <XAxis
            dataKey="date"
            tick={{ fontSize: 10, fill: 'rgba(156,163,175,0.7)', fontFamily: 'JetBrains Mono, monospace' }}
            tickLine={false}
            axisLine={false}
            interval={tickStep}
          />
          ...
          <Legend
            ...
            formatter={(value: string) => (
              <span style={{ color: value === 'Retorno' ? '#00D4FF' : '#8B5CF6', fontSize: '12px' }}>
                {value}
              </span>
            )}
          />
          ...
          <Area
            ...
            dot={false}
            activeDot={{ r: 4, fill: '#8B5CF6', stroke: '#030712', strokeWidth: 2 }}
          />
          ...
          <Area
            ...
            dot={false}
            activeDot={{ r: 4, fill: '#00D4FF', stroke: '#030712', strokeWidth: 2 }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
