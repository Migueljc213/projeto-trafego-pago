...
interface CustomTooltipProps {
  ...
  label?: string
  t: Dictionary['dashboardHome']['revenueChart']
}

function CustomTooltip({ active, payload, label, t }: CustomTooltipProps) {
  if (!active || !payload || payload.length === 0) return null
  const investment = payload.find(p => p.name === t.seriesInvestment)
  const revenue = payload.find(p => p.name === t.seriesReturn)
  ...
  return (
    <div className="bg-dark-card border border-gray-700 rounded-lg px-3 py-2.5 text-xs shadow-xl">
      ...
      {revenue && <p style={{ color: '#00D4FF' }} className="font-semibold mb-0.5">{t.tooltipReturn} {formatCurrency(revenue.value)}</p>}
      {investment && <p style={{ color: '#8B5CF6' }} className="font-medium mb-0.5">{t.tooltipInvest} {formatCurrency(investment.value)}</p>}
      <p className="text-gray-400 mt-1">{t.tooltipRoas} <span className="text-white font-bold">{roas}x</span></p>
    </div>
  )
}
...
export default function RevenueChart({ data }: { data: RevenueDataPoint[] }) {
  ...
  const t = dict.dashboardHome.revenueChart
    ...
    <div className="glass-card rounded-xl p-5 border border-gray-800">
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="px-3 py-1.5 rounded-lg bg-neon-cyan/10 border border-neon-cyan/20">
          <span className="text-xs text-gray-400">{t.totalReturn} </span>
        ...
        <div className="px-3 py-1.5 rounded-lg bg-neon-purple/10 border border-neon-purple/20">
          <span className="text-xs text-gray-400">{t.totalInvestment} </span>
        ...
        <div className="px-3 py-1.5 rounded-lg bg-white/5 border border-gray-700">
          <span className="text-xs text-gray-400">{t.avgRoas} </span>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          ...
          <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '12px' }} formatter={(value: string) => (
            <span style={{ color: value === t.seriesReturn ? '#00D4FF' : '#8B5CF6', fontSize: '12px' }}>{value}</span>
          )} />
          <Area type="monotone" dataKey="investment" name={t.seriesInvestment} stroke="#8B5CF6" strokeWidth={1.5} fill="url(#purpleGradient)" dot={false} activeDot={{ r: 4, fill: '#8B5CF6', stroke: '#030712', strokeWidth: 2 }} />
          <Area type="monotone" dataKey="revenue" name={t.seriesReturn} stroke="#00D4FF" strokeWidth={2} fill="url(#cyanGradient)" dot={false} activeDot={{ r: 4, fill: '#00D4FF', stroke: '#030712', strokeWidth: 2 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
