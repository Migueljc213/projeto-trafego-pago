...
export async function generateMetadata() {
  ...
  return { title: dict.audit.precosPage.metaTitle }
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-white">{dict.audit.precosPage.title}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{dict.audit.precosPage.subtitle}</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg glass-card border border-gray-700">
          ...
          <span className="text-xs text-gray-400 font-medium">{dict.audit.precosPage.crawlerActive}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="glass-card rounded-xl p-4 border border-gray-800">
          <p className="text-xs text-gray-500 mb-1">{dict.audit.precosPage.stats.monitoredCompetitors}</p>
          <p className="text-2xl font-bold font-mono text-neon-cyan">{competitors.length}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-gray-800">
          <p className="text-xs text-gray-500 mb-1">{dict.audit.precosPage.stats.withPriceCollected}</p>
          <p className="text-2xl font-bold font-mono text-green-400">
            {competitors.filter(c => c.lastPrice !== null).length}
          </p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-gray-800">
          <p className="text-xs text-gray-500 mb-1">{dict.audit.precosPage.stats.priceAlerts}</p>
          ...
          {competitors.length > 0 && (
            <div className="glass-card rounded-xl border border-gray-800 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-800">
                <h3 className="text-sm font-semibold text-white">{dict.audit.precosPage.registeredCompetitors}</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800 bg-white/2">
                      <th className="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">{dict.audit.precosPage.table.name}</th>
                      <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">{dict.audit.precosPage.table.url}</th>
                      <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">{dict.audit.precosPage.table.lastPrice}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">{dict.audit.precosPage.table.checked}</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={comp.id} className={`border-b border-gray-800/50 ${idx % 2 === 0 ? 'hover:bg-white/2' : ''}`}>
                        ...
                        <td className="px-4 py-3.5 text-right font-mono font-semibold text-white">
                          {comp.lastPrice !== null
                            ...
                            : <span className="text-gray-600 text-xs">{dict.audit.precosPage.table.pending}</span>
                          }
                        </td>
