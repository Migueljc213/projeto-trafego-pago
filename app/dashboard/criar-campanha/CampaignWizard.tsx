          ...
          <div className="space-y-5">
            ...
            <div className="p-4 rounded-xl border border-blue-500/20 bg-blue-500/5">
              ...
              <p className="text-xs text-gray-500 mb-2">
                Cole IDs de interesses da Meta separados por vírgula, ou deixe em branco para alcance amplo.
              </p>
              <Input
                placeholder="Ex: 6003002795430 (Empreendedorismo), 6003139266461 (Marketing Digital)"
                value={form.interests?.map(i => i.id).join(', ') ?? ''}
                onChange={e => {
                  const ids = e.target.value.split(',').map(s => s.trim()).filter(Boolean)
                  update('interests', ids.map(id => ({ id, name: id })))
                }}
              />
            </div>
          </div>
